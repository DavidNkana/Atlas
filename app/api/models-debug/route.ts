...
import { fetchOpenRouterFreeModelIds } from "@/lib/models/openrouter-discovery";
 ...
 *
 * Diagnostic endpoint. Surfaces what Atlas actually sees right now so the
 * operator can verify which OpenRouter free models are available without
 ...
 *   - atlasSelected:       Array<{id, displayName, provider, free, available}>
 */
export async function GET() {
  const freeIds = await fetchOpenRouterFreeModelIds();
