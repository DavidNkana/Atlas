...
// LCP-45 — Tighter filter. The previous lists still let
// through noise: "A Group Of Guys Move Into The Apartment
// Upstairs" (Twistedsifter), "Iran war cost" (CNN), "Nasty
// upstate Republican primary" (Gothamist), "A new survey on
// dads" (NPR), "A New Neighbor's Leaky Trucks...Apartment
// Complex" (Twistedsifter). These all matched a term like
// "apartment" or "Home" but they're not real estate.
//
// We now require the article to have a strong real-estate
// signal in BOTH the title AND the description, AND a strong
// real-estate phrase (multi-word preferred) in at least one.
const STRONG_REAL_ESTATE_TERMS = [
  ...
  "zoning",
  "reit",
  // Less specific but still strongly real-estate-only
  "buy a home",
  "buy a house",
  "sell a home",
  "sell a house",
...
function isRealEstateArticle(article: NewsArticle): boolean {
  ...
  // Whitelist: well-known real estate publications
  const isRealEstateSource = [
    "commercial observer",
    "realtor.com",
    "zillow",
    "redfin",
  ...
  if (isRealEstateSource) return true;

  // General: require at least one STRONG real-estate term in
  // title+description. Single-word terms like "reit" still
  // count because they're concrete enough.
  return STRONG_REAL_ESTATE_TERMS.some((term) => haystack.includes(term));
}
